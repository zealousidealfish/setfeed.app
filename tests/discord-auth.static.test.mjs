...
for (const code of ['access_denied', 'invalid_or_expired_state', 'invalid_or_expired_result', 'discord_exchange_failed', 'discord_profile_failed', 'discord_not_linked', 'firebase_account_mismatch', 'firebase_user_unavailable', 'rate_limited', 'invalid_firebase_token', 'internal_error', 'malformed_json_response', 'invalid_authorization_url', 'firebase_custom_token_sign_in_failed', 'ambiguous_exchange_outcome']) {
...
includes(moduleText, 'json.error && typeof json.error.code === "string"', 'nested backend error.code is authoritative');
includes(moduleText, 'typeof json.error === "string"', 'legacy string backend errors remain defensively supported');
includes(moduleText, 'stableErrorCode(json) || responseFallbackCode(response)', 'start and exchange use shared stable-code parser with safe fallback');
includes(moduleText, 'url.port', 'authorization URL validation rejects non-empty ports');
...
includes(completeHtml, 'let restartDestination = window.SetfeedDiscordAuth.DEFAULT_DESTINATION;', 'completion page has a default restart destination');
includes(completeHtml, 'if (state.ok) restartDestination = state.destination;', 'callback errors preserve a validated restart destination');
includes(completeHtml, 'startDiscordSignIn({ auth, destination: restartDestination })', 'restart starts a fresh OAuth flow with the preserved safe destination');
