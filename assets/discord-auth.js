(function () {
  ...
  function validateAuthorizationUrl(value) {
    ...
    if (url.protocol !== "https:" || url.hostname !== "discord.com" || url.pathname !== "/oauth2/authorize" || url.username || url.password || url.hash) return false;
  ...
  async function currentIdToken(auth) { return auth && auth.currentUser && typeof auth.currentUser.getIdToken === "function" ? auth.currentUser.getIdToken() : ""; }
  async function startDiscordSignIn({ auth, destination } = {}) {
    ...
    const token = await currentIdToken(auth); if (token) headers.Authorization = `Bearer ${token}`;
    let response;
    ...
    if (!response.ok) { const code = json && typeof json.error === "string" ? json.error : (response.status >= 500 ? "backend_unavailable" : "internal_error"); clearState(); throw { code, retryAfter: parseRetryAfter(response) }; }
  ...
  async function exchangeDiscordResult({ auth, resultCode } = {}) {
    ...
    const headers = { "Content-Type":"application/json" }; const token = await currentIdToken(auth); if (token) headers.Authorization = `Bearer ${token}`;
    let response;
