...
15. Consumed token: complete an already used token; verify the already-used message appears.
16. Replaced/cancelled or inactive token: complete invalidated and inactive test tokens; verify each state asks for another Discord link.
17. Account conflict: complete tokens for `conflicting_discord_account` and `conflicting_firebase_uid`; verify each conflict message names the conflicting side.
18. Authentication backend responses: force `invalid_firebase_token`, `missing_auth`, and `anonymous_firebase_user`; verify the page asks for sign-in again or a full Google/email account as appropriate.
19. Rate limit and wrong origin: force `rate_limited` and `wrong_origin`; verify the page asks the user to wait or reports configuration safely.
20. Temporary service error: point a local test build at a backend that returns 503; verify the temporary-service-error message appears and the button is re-enabled.
21. Mobile layout: verify panels, buttons, checkbox label, and status messages fit at 320px width without horizontal scrolling.
22. Privacy check: verify browser requests never include a Discord user ID field and that the token is not written to localStorage.
